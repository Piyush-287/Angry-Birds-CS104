...
def display_tower(screen:pygame.surface,curr,size):
    ...
    for i in range(size[0]):
        for j in range(size[1]):
            ...
            screen.blit(Blocks[curr[j*size[0]+i]],posn)
...
def redraw_screen():
    ...
    pygame.draw.rect(screen, BROWN, (0, 450, screen_width, screen_height - 450))
...
def get_input(initial_input):
    ...
    while running:
        screen_width, screen_height = screen.get_size()
        screen.fill(WHITE)
        RATIO= 0.45/ max(tile_map_size[0],tile_map_size[1])
        ...
        display_tower(screen,Designs[curr_index],tile_map_size)
        pygame.display.flip()
        clock.tick(60)
    return player_name,curr_index
